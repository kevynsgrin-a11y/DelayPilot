...
**Re-review round (on the fix-loop tree `692ad3f`).**

| Review                             | Verdict                                                                                                                                                                                                                                                                                                         | Residue                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Accessibility re-review (`§15.15`) | **GREEN** — B8 and B9 closed by measurement (20 of 20 routes at exactly 320 px; the meter name confirmed in the accessibility tree); 88 axe runs, 0 violations, 0 best-practice; F25–F38 walked, F31's caption re-owned to content                                                                              | F39 (leg pills announce "Watch Status"), F41 (feedback address target size on the conditional route), F42 (loading message announced twice) → `frontend-ui-engineer`; F40 (the statement's known-issues list must follow the GREEN verdict) → `ux-copy-steward`; prose-table captions → `content-editorial-lead`; the `§12` assertions → `qa-test-architect`                                                                                                                                               |
| Copy re-check                      | PASS WITH FINDINGS — F-1…F-12, F-13, F-14, F26, F36, F37 all closed in the built tree; rendered-text and attribute scans 0 hits                                                                                                                                                                                 | F-16…F-20 fixed in the copy module (a flat determination in the negative direction, a superlative, an example that modelled the rejected entry, an overclaiming registry note, a typo); F-21…F-26 → `frontend-ui-engineer` (the stale-state card's demo sentence and the widened guard, the flight-data disclaimer per card group, a duplicated §27 sentence, a flat determination in the demo fixture, three generic alert bodies, a non-sequitur deadline reason); F-27, F-28 → `content-editorial-lead` |
| Trust re-check                     | F1–F6 **closed**; disclaimers, false affiliation, ad placement, affiliate disclosure **pass**; dark patterns pass on the two families that have a surface; legal overclaim **FAIL — 1 critical** (F8, the same negative-direction determination the copy re-check fixed as F-16, closed only on a rebuilt tree) | F9 (the lint is defeated by one word inserted between "the" and "airline") → `ux-copy-steward`; F10 (the independence disclaimer is dropped from print), F11 (§20 clause 5 asserted only through containers), F12 (= copy F-21), F13 (two join-guard exclusions justified by content, not layout) → `frontend-ui-engineer`; rulings: the route-timeline panel is a value panel, the stale-state card must carry the sentence under `DIRECTIVE.md §28`                                                      |

Ruling adopted (`DIRECTIVE.md §3.1`): the demo sentence is triggered by fixture data, not by the word on the chip — `docs/VOICE.md §2.1` and the trust officer's reading of §28 agree — so the guard keys on fixture-sourced panels. **Residue outcome.** Every routed item landed: copy (trust F9 with fourteen elastic lint rules and the pinned regression, F-16…F-20, F-25's alert bodies, F40), content (F-27 in five entries, F-28's label column, `EDITORIAL_POLICY.md §6.5`), frontend (trust F10–F13, copy F-21…F-26, accessibility F39, F41, F42). Trust F8 is closed on the rebuilt tree: the sentence is absent from the built `/connection-risk/` and the hedged form is present (orchestrator-measured; the trust officer's own read-back is the last step of the round). Still open by design: F15 and F23 (brand, by decision), the prose-table captions (content, a remark plugin or layout wrapper), `severityMeanings` deleted by the steward now that its last call sites are gone, and the negative-determination lint class ("no airline is responsible", "is not liable") which the fixture test holds as a stopgap.

**Read-backs on the residue tree (`af24302`).** `trust-compliance-officer`: all six verdicts **PASS** (dark patterns on the two families that have a surface); F8 closed at the byte level and the directive's own §15.3 correction endorsed as the root-cause fix; F9–F13 closed; three new minors F14–F16 confined to lint coverage (a negation-aware gap for `bare-guarantee`, the GAP docblock's clause claim against a full stop, four prospective trees for `VOICE_TREES`) routed to `ux-copy-steward` in the same pass; a standing recommendation that the negated determination be carried as an overclaim class. `accessibility-lead` (`§15.16`): **GREEN unchanged**; F39, F41, F42 closed by measurement (39 pills with no field label, the address at 47 px tall, one announcement); 80 + 8 axe runs at 0; reflow exact on 22 of 22; the statement may publish with I-3 once its list is exactly F15, F23, F31 with the reviewed date moved to 2026-09-13 (`ux-copy-steward`, same pass). Open by decision or waiting on an owner: F15, F23, F31's captions, F40 until the four edits land.

**Final copy pass.** `severityMeanings` deleted; twelve negative-determination lint rules scoped to the reader-facing trees (the claim map included, with its ruling in `docs/VOICE.md §4.5.1`: a proposition is attributed, not hedged); the rule found and closed F-29 in the module's own `/connection-risk/` sentence; trust F14–F16 closed (a negation-aware gap for `bare-guarantee`, a sentence-ending full stop that a gap cannot cross, the four engine trees scoped before they exist); the accessibility statement carries exactly the four authorised edits and publishes only with I-3 after the accessibility lead's last read. Final tree: 791 tests, 39 lint rules, `verify-dist --self-test` 28 checks, 0 findings; every gate below re-run and Passing.
...
- `frontend-ui-engineer` + `ux-copy-steward` — the five rights-status labels are retyped in `passenger-rights/overview.md` (copy re-check F-28) because Markdown bodies have no copy slot: add a `{{copy:…}}` slot resolved in `ArticleLayout.astro`, or a test asserting the table against `cockpit.rights.statusLabels`; a real `<caption>` on prose tables needs a remark plugin or a layout wrapper (the caption text exists as each table's lead-in sentence).
- `regulatory-source-steward` — `claim-map.json` keeps the flat proposition "does not create an obligation on the second" as the thing to verify while the body sentence is hedged; align the map wording if the verified source supports the categorical form.

...
| `docs/agents/**`, `docs/BUILD_PLAN.md`, `scripts/validate-build-system.mjs`                                                                                                                                                                                                                                                                         | `build-orchestrator`                                                                                             |
...
| `apps/web/src/styles/**`, `packages/ui/src/primitives/**`, `packages/ui/src/tokens/**`, `apps/web/public/fonts/**`, `scripts/validate-contrast.mjs`                                                                                                                                                                                                 | `brand-design-director`                                                                                          |
| `apps/web/public/brand/**`, `apps/web/public/icons/**`, `apps/web/public/og/**`, `apps/web/public/images/**`, `scripts/assets/**`, `design/**`                                                                                                                                                                                                      | `visual-asset-director`                                                                                          |
| `apps/web/src/components/**`, `apps/web/src/islands/**`, `apps/web/src/layouts/**`, `apps/web/src/app/**`, `apps/web/src/lib/markdown/**`, `apps/web/astro.config.mjs`, `apps/web/scripts/**`, `apps/web/package.json`, `packages/ui/src/patterns/**`                                                                                               | `frontend-ui-engineer`                                                                                           |
...
| `docs/PERFORMANCE.md`, `scripts/perf/**`, `perf.budgets.json`                                                                                                                                                                                                                                                                                       | `performance-engineer`                                                                                           |
| `apps/edge/wrangler.jsonc`, `apps/edge/worker-configuration.d.ts` (generated by `wrangler types`, never hand-edited), `vercel.json`, `apps/web/vercel.json`, `apps/web/public/_headers`, `.github/workflows/**`, `scripts/validate-security-headers.mjs`, `packages/observability/**`, `docs/DEPLOYMENT.md`, `docs/RUNBOOK.md`, `docs/ANALYTICS.md` | `platform-release-sre`                                                                                           |
...
security suites, and may _read_ everything.

**Enumerated shared-surface exceptions** (the only permitted deviations from single-writer
ownership, per `AGENTS.md §3.5`):

1. **Phase 1 scaffolding.** `principal-architect` creates the _empty shell_ of every workspace
   ...
   to its roster owner and `principal-architect` files handoffs like anyone else. A workspace
   app's own `package.json` is part of that shell, so `apps/web/package.json` belongs to
   `frontend-ui-engineer` and the root `package.json` stays `principal-architect`'s. That was
   left to inference until S4, when an agent had to read `git log` to find out; the table now
   says it, because a path whose owner can only be discovered by archaeology is an unowned path.
...
3. **Copy inside another agent's file.** `ux-copy-steward` owns the _strings_ in
